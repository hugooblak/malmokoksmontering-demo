...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'malmokoksmontering-leads' ),
					'singular_name' => __( 'Lead', 'malmokoksmontering-leads' ),
					'edit_item'     => __( 'Förfrågan', 'malmokoksmontering-leads' ),
					'search_items'  => __( 'Search leads', 'malmokoksmontering-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'malmokoksmontering-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'malmokoksmontering-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'malmokoksmontering-leads' ),
			'npl_phone'    => __( 'Phone', 'malmokoksmontering-leads' ),
			'npl_service'  => __( 'Needs', 'malmokoksmontering-leads' ),
			'npl_timeline' => __( 'When', 'malmokoksmontering-leads' ),
			'npl_zip'      => __( 'ZIP', 'malmokoksmontering-leads' ),
			'npl_source'   => __( 'Source', 'malmokoksmontering-leads' ),
			'npl_status'   => __( 'Status', 'malmokoksmontering-leads' ),
			'date'         => __( 'Received', 'malmokoksmontering-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'malmokoksmontering-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'malmokoksmontering-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'malmokoksmontering-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'malmokoksmontering-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'malmokoksmontering-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'malmokoksmontering-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'malmokoksmontering-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'malmokoksmontering-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'malmokoksmontering-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'malmokoksmontering-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'malmokoksmontering-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'malmokoksmontering-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
